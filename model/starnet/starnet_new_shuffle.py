...
def channel_shuffle(x, groups):
    ...
    # transpose groups and channels within group
    x = x.transpose(1, 2).contiguous()
    # flatten back to (B, C, H, W)
    x = x.view(B, C, H, W)
    return x

class FFN_GROUP(torch.nn.Module):
    def __init__(self, ed, h, group):
        ...
        self.pw1 = Conv2d_BN(ed, h, groups=self.groups)
        self.act = torch.nn.GELU()
        self.pw2 = Conv2d_BN(h, ed, groups=self.groups, bn_weight_init=0)

    def forward(self, x):
        x = self.pw1(x)           # 分组升维
        x = channel_shuffle(x, self.groups)  # 👈 关键：引入 shuffle
        x = self.act(x)
        x = self.pw2(x)           # 分组降维（现在能看到混合信息）
        return x
...
class Block(nn.Module):
    def __init__(self, dim, ffn_ratio, drop_path=0., wt_type='db1', learnable_wavelet=False,stage=0, shuffle_group=1):
        super().__init__()
        self.DW = RepVGGDW(dim)
        self.ese = EffectiveSELayer(dim)
        if shuffle_group > 1:
            self.ffn1 = Residual(FFN_GROUP(dim, int(dim*ffn_ratio), group=shuffle_group),drop=0)
        else:
        ...
        if shuffle_group > 1:
            self.ffn2 = Residual(FFN_GROUP(dim, int(dim*ffn_ratio), group=shuffle_group),drop=0)
        else:
...
class FSANet(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, dims=[40,80,160,320], shuffle_groups=[1,2,2,2], depth=[1,2,4,5], mlp_ratio=2., act_layer="GELU",drop_path_rate=0., distillation=False, head_init_scale=0. ,layer_scale_init_value=0., learnable_wavelet=True,down_sample=32):
        super().__init__()
        # if act_layer == "GELU":
        #     act_layer = nn.GELU
        # elif act_layer == "ReLU":
        ...
        for i, (dim, dpth) in enumerate(
                            zip(dims,depth)):
            for j in range(dpth):
                blocks[i].append(Block(dim,ffn_ratio=mlp_ratio, wt_type='db1',drop_path=drop_path_rate, learnable_wavelet=learnable_wavelet,stage=i, shuffle_group=shuffle_groups[i]))
...
#运算量：229.647M, 参数量：3.267M
def FSANet_T2_Shuffle(num_classes=1000, pretrained=False, distillation=False, fuse=False, pretrained_cfg=None, model_cfg=CFG_StarAttn_T2):
    model = FSANet(num_classes=num_classes, distillation=distillation, **model_cfg)
    return model

#@MODEL.register_module
#运算量：437.306M, 参数量：6.125M
def FSANet_T4_Shuffle(num_classes=1000, pretrained=False, distillation=False, fuse=False, pretrained_cfg=None, model_cfg=CFG_StarAttn_T4):
    model = FSANet(num_classes=num_classes, distillation=distillation, **model_cfg)
    return model
