...
class StarNet_MHSA(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, dims=[40,80,160,320], depth=[1,2,4,5], mlp_ratio=2., shuffle_groups = [1,2,2,4],act_layer="GELU",drop_path_rate=0., distillation=False, head_init_scale=0. ,layer_scale_init_value=0., learnable_wavelet=True,down_sample=32):
        super().__init__()
        # if act_layer == "GELU":
        #     act_layer = nn.GELU
        # elif act_layer == "ReLU":
...
def StarNet_MHSA_T2_DTW_Shuffle(num_classes=1000, pretrained=False, distillation=False, fuse=False, pretrained_cfg=None, model_cfg=CFG_StarAttn_T2):
...
@MODEL.register_module
def StarNet_MHSA_T8_Shuffle(num_classes=1000, pretrained=False, distillation=False, fuse=False, pretrained_cfg=None, model_cfg=CFG_StarAttn_T8):
    model = StarNet_MHSA(num_classes=num_classes, distillation=distillation, **model_cfg)
    return model
